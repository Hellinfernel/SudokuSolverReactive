...
public class NumberGroup {
    ...
    public NumberGroup(Field[] matrix, ExecutorService executorService) throws ArrayIndexOutOfBoundsException {
        ...
        _numberFields.forEach(field -> field._changeInPossibleNumbersEvent.addFunction(integer -> _executorService.execute(this::searchSetableNumber)));
        _numberFields.forEach(field -> field._trueNumberFoundEvent.addFunction(integer -> _executorService.execute(() -> this.excludeNumber(integer))));




    }
    public NumberGroup(int[] matrix, ExecutorService executorService)throws ArrayIndexOutOfBoundsException{
        _leftNumbers = Collections.synchronizedSet(new HashSet<>(Constants.ALL_NUMBERS));
        _executorService = executorService;
    ...
    public void searchSetableNumber() {
        for (Integer number : _leftNumbers) {
            ...
            if (fields.size() == 1){
                ...
                _executorService.execute(() -> field.setNumber(number));
    ...
    private void excludeNumber(int number) {
        ...
        for (Field field : _numberFields) {
            if (field.isFixed() == false) {
                _executorService.execute(() -> field.exclude(number));
            }
        }
