...
import newAlternative.field.EmptyField;
import newAlternative.field.Field;
import newAlternative.field.StaticField;
...
public class NumberGroup {
    ...
    private final TaskSchedular<NumberGroup> _taskSchedular = new TaskSchedular<>(this);
    ...
    public void searchSetableNumber() {
        for (Integer number : _leftNumbers) {
            ...
            if (fields.size() == 1){
                fields.get(0).giveTask(new Task<Field>(field -> field.setNumber(number),  (byte) 10));
    ...
    private void excludeNumber(int number) {
        ...
        for (Field field : _numberFields) {
            if (field.isFixed() == false) {
                field.giveTask(new Task<Field>(field1 -> field1.exclude(number), (byte) 1));
            }
        }
